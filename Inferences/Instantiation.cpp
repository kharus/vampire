...
void Instantiation::registerClause(Clause* cl)
{
  ...
  //cout << "register " << cl->toString() << std::endl;
    ...
    while(it.hasNext()){
      ...
      if(t.isTerm() && t.term()->ground()){
        ...
        if(SortHelper::tryGetResultSort(t,sort)){
          ...
          ASS(cans_check && cans);
          // cout << "For sort " << sort << " there are " << cans->size() << " cans" <<std::endl;
...
public:
  ...
  Substitution next()
  {
    ...
    //cout << "sub is " << sub.toString() << std::endl;
...
{
  //cout << "Instantiate " << premise->toString() << std::endl;
