...
#if DPRINT
    cout << "special lit " << lit -> toString() << std::endl;
#endif
    return false;
  }
...
#if DPRINT
  cout << "found pure literal: " << lit->toString() << std::endl;
#endif
  return true;
}
...
{
#if DPRINT
  cout << "selecting trivial literals in " << cl->toString() << std::endl ;
#endif
  ...
  while( it.hasNext() ) {
    ...
    if (isPure(c)) {
      ...
      if (c->isNegative()
          && c->isEquality()) {
#if DPRINT
        cout << "checking " << c->toString() << std::endl;
#endif
...
#if DPRINT
        cout << "non trivial pure found " << c->toString() << std::endl;
#endif
        nontriv_pure.push(c);
      }
    } else { // !isPure(c)
...
#if DPRINT
  cout << "Found " << triv_candidates.length() << " trivial candidates." << std::endl;
  cout << "Found " << nontriv_pure.length() << " nontrivial pure literals." << std::endl;
  cout << "Found " << impure.length() << " impure literals." << std::endl;
#endif
...
#if DPRINT
  cout << "Found " << triv_candidates.length() << " trivial literals." << std::endl;
#endif
...
#if DPRINT
  cout << "selectTheoryLiterals in " << cl->toString() << std::endl;
#endif
...
#if DPRINT
  cout << "div zero filtering checking!" << std::endl;
#endif
  Stack<Literal*>::BottomFirstIterator it(theoryLits);
  while(it.hasNext()) {
    Literal* lit = it.next();
#if DPRINT
    cout << "div zero filtering checking: " << lit->toString() << std::endl;
#endif
    bool keep_lit = true;
      ...
      if(ts.isTerm()){
        ...
        if( theory->isPartiallyInterpretedFunction(t)
         && theory->partiallyDefinedFunctionUndefinedForArgs(t)
          ...
          keep_lit = false;
#if DPRINT
          cout << "division by zero removed: " << lit->toString() << std::endl;
#endif
        }
      }
    }
