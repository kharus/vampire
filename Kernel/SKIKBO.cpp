...
bool SKIKBO::safe(Term* t1, Term* t2) const 
{
  ...
  while(!toBeChecked.isEmpty()){
    ...
    //cout << term1.toString() << std::endl;
    //cout << term2.toString() << std::endl;
...
SKIKBO::VarCondRes SKIKBO::compareVariables(TermList tl1, TermList tl2) const
{
  ...
  while(tl2vit.hasNext()){
    ...
    //cout << "its multip on right is " << tl1Mult << std::endl;
    if(tl1Mult > tl2Mult && vcr != RIGHT){
      vcr = LEFT;
    } else if(tl2Mult > tl1Mult && vcr != LEFT){
      vcr = RIGHT;
...
Ordering::Result SKIKBO::compare(TermList tl1, TermList tl2) const
{
  ...
  /*if(print){
    if(res == LESS){ cout << tl1.toString() + " < " + tl2.toString() << std::endl; }
    if(res == GREATER){ cout << tl1.toString() + " > " + tl2.toString() << std::endl; }
    if(res == INCOMPARABLE){ cout << tl1.toString() + " <> " + tl2.toString() << std::endl; }
  }*/
#if VDEBUG
  _state=state;
#endif
