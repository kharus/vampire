...
Term* BottomUpTermTransformer::transform(Term* term)
{
  ...
  // cout << "transform " << lit->toString() << std::endl;
  ...
  for(;;) {
    ...
    if(tt->isEmpty()) {
      ...
      // cout << "term popped " << orig->toString() << std::endl;
      ...
      // cout << "args.length() - orig->arity() = " << args.length() - orig->arity() << std::endl;
      if(orig->isSort()){
        //For most applications we probably dont want to transform sorts
        //however, we don't enforce that here, inheriting classes can decide
        //for themselves
    ...
    // cout << "Non-empty: " <<  tt->toString() << std::endl;
