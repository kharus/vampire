...
void BlockedClauseElimination::apply(Problem& prb)
{
  ...
  // cout << "Queue initialized" << std::endl;
  ...
  while (!queue.isEmpty()) {
    ...
    if (env.options->showPreprocessing()) {
      cout << "[PP] Blocked clause[" << cand->litIdx << "]: " << cl->toString() << std::endl;
    }
...
bool BlockedClauseElimination::resolvesToTautologyEq(Clause* cl, Literal* lit, Clause* pcl, Literal* plit)
{
  ...
  // cout << "cl: " << cl->toString() << std::endl;
  // cout << "lit: " << lit->toString() << std::endl;
  // cout << "pcl: " << pcl->toString() << std::endl;
  // cout << "plit: " << plit->toString() << std::endl;
...
bool BlockedClauseElimination::resolvesToTautologyEq(Clause* cl, Literal* lit, Clause* pcl, Literal* plit)
{
  ...
  for (unsigned i = 0; i < cl->length(); i++) {
    ...
    if (curlit->functor() != lit->functor() || curlit->polarity() != lit->polarity()) {
      ...
      // cout << "norm_oplit1: " << norm_oplit->toString() << std::endl;
  ...
  for (unsigned i = 0; i < pcl->length(); i++) {
    ...
    if (curlit->functor() != plit->functor() || curlit->polarity() != plit->polarity()) {
      ...
      // cout << "norm_oplit2: " << norm_oplit->toString() << std::endl;
...
bool BlockedClauseElimination::resolvesToTautologyUn(Clause* cl, Literal* lit, Clause* pcl, Literal* plit)
{
  ...
  for (unsigned i = 0; i < cl->length(); i++) {
    ...
    // cout << "insert1(scurlit): " << scurlit->toString() << std::endl;
  }

  // cout << "opslit: " << opslit->toString() << std::endl;
  ...
  for (unsigned i = 0; i < pcl->length(); i++) {
    ...
    // cout << "insert2(scurlit): " << scurlit->toString() << std::endl;
