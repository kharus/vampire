...
Clause* TheoryFlattening::apply(Clause*& cl,Stack<Literal*>& target)
{
  ...
  while(!lits.isEmpty()){
    ...
    if(nlit==lit){
      ...
      //cout << lit->toString() << " flattened to " << nlit->toString() << std::endl; 
      //for(unsigned i=0;i<newLits.length();i++){ cout << ">> " << newLits[i]->toString() << std::endl; }
      updated=true;
      if(_recursive){
        lits.push(nlit);
 ...
 */
 Literal* TheoryFlattening::replaceTopTerms(Literal* lit, Stack<Literal*>& newLits,unsigned& maxVar,
                                            DHMap<Term*,unsigned>& abstracted)
{
  //cout << "replaceTopTerms " << lit->toString() << std::endl;
  ...
  //cout << "interpreted is " << interpreted << std::endl;
  ...
  for(TermList* ts = lit->args(); ts->isNonEmpty(); ts = ts->next()){
    ...
    //cout << "term " << t->toString() << " has interp=" << env.signature->getFunction(t->functor())->interpreted() << std::endl;
    ...
    if(
      ...
      //cout << "Factoring out " << t->toString() << std::endl;
      ...
      //cout << "recurse in  " << tt->toString() << std::endl;
 ...
 */
 Term* TheoryFlattening::replaceTopTermsInTerm(Term* term, Stack<Literal*>& newLits,
                                               ...
                                               DHMap<Term*,unsigned>& abstracted)
{
  //cout << "replaceTopTermsInTerm " << term->toString() << std::endl;
  ...
  //cout << "updated is " << updated << std::endl;
