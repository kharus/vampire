...
void SortInference::doInference()
{
  ...
  if(_ignoreInference){
   ...
   cout << "Ignoring sort inference..." << std::endl;
#endif
    ...
    for(unsigned f=0;f<env.signature->functions();f++){
      ...
      //cout << env.signature->functionName(f) << " : " << env.sorts->sortName(ftype->result()) << std::endl;;
      TermList resTypeT = ftype->result();
      unsigned resType = resTypeT.term()->functor();
      unsigned dsort = (*_sig->vampireToDistinct.get(resType))[0];
      //cout << env.signature->functionName(f) << " : " << dsort << std::endl;
      if(arity==0){
...
#if DEBUG_SORT_INFERENCE
       cout << "Skipping deleted function signature for " << env.signature->functionName(f) << std::endl;
#endif
        continue;
      }
      unsigned arity = env.signature->functionArity(f);
  ...
  if(_usingMonotonicity){
    if(_print){
      cout << "Monotonicity information:" << std::endl;
      if(_assumeMonotonic){ cout << "Assuming all sorts monotonic due to translation" << std::endl; }
    }
    for(unsigned s=0;s<env.signature->typeCons();s++){
      if(env.getMainProblem()->getProperty()->usesSort(s) || env.signature->isNonDefaultCon(s)){
        ...
        if(_print){
          if(monotonic && !_assumeMonotonic){
            cout << "Input sort " << env.signature->typeConName(s) << " is monotonic" << std::endl;
          }
        }
      }
    }
...
#if DEBUG_SORT_INFERENCE
  //cout << "just functions count is " << count << std::endl;
#endif
...
#if DEBUG_SORT_INFERENCE
  cout << "count is " << count << ": this is the number of positions in the considered signature" << std::endl;
#endif
  ...
  for(unsigned i=0;i<c->length();i++){
...
#if DEBUG_SORT_INFERENCE
        cout << "join X" << l->nthArgument(0)->var()<< " and X" << l->nthArgument(1)->var() << std::endl;
#endif
...
#if DEBUG_SORT_INFERENCE
          cout << "varsWithPosEq X" << l->nthArgument(0)->var() << std::endl;
          cout << "varsWithPosEq X" << l->nthArgument(1)->var() << std::endl;
#endif
        }         
      } else {
...
#if DEBUG_SORT_INFERENCE
        cout << "push " << n << " for X" << l->nthArgument(1)->var() << std::endl;
#endif
        for(unsigned i=0;i<t->arity();i++){
          ASS(t->nthArgument(i)->isVar());
          varPositions[t->nthArgument(i)->var()].push(n+1+i);
#if DEBUG_SORT_INFERENCE
          cout << "push " << (n+1+i) << " for X" << t->nthArgument(i)->var() << std::endl;
#endif
        }
        if(l->polarity()){
...
#if DEBUG_SORT_INFERENCE
        cout << "push " << (n+i) << " for X" << l->nthArgument(i)->var() << std::endl;
#endif
      }
    }
  } 
...
#if DEBUG_SORT_INFERENCE
   cout << "num_vars_always_in_equalities: " << num_vars_always_in_equalities << std::endl;
...
#if DEBUG_SORT_INFERENCE
     cout << "Max size for sort " << sort << " is " << c->length() << std::endl;
#endif
   }
   ...
   for(unsigned v=0;v<varPositions.size();v++){
     ...
     for(unsigned i=0;i<stack.size();i++){
       if(varsWithPosEq[v]){
#if DEBUG_SORT_INFERENCE
         cout << "recording posEq for " << stack[i] << std::endl;
#endif
         posEqualitiesOnPos[stack[i]]=true;
       }
       for(unsigned j=i+1;j<stack.size();j++){
#if DEBUG_SORT_INFERENCE
         cout << "doing union " << stack[i] << " and " << stack[j] << std::endl;
#endif
...
#if DEBUG_SORT_INFERENCE
  cout << comps << " components: this is the number of disjoint subsorts" << std::endl;
#endif
  ...
  for(unsigned f=0;f<env.signature->functions();f++){
    ...
    if(arity==0){
#if DEBUG_SORT_INFERENCE
    cout << "adding " << env.signature->functionName(f) << " as constant for " << rangeSort << std::endl;
    //cout << "it is " << Term::createConstant(f)->toString() << std::endl;
#endif
       _sig->sortedConstants[rangeSort].push(f);
    }
    else{
#if DEBUG_SORT_INFERENCE
      cout << "recording " << env.signature->functionName(f) << " as function for " << rangeSort << std::endl;
#endif
  ...
  if(_print){
    cout << "Sort Inference information:" << std::endl;
    cout << comps << " inferred subsorts" << std::endl;
  }
  unsigned firstFreshConstant = UINT_MAX;
  DHMap<unsigned,unsigned> freshMap;
  for(unsigned s=0;s<comps;s++){
#if DEBUG_SORT_INFERENCE
      if(!posEqualitiesOnSort[s]){ cout << "No positive equalities for subsort " << s << std::endl; }
#endif
...
#if DEBUG_SORT_INFERENCE
      cout << "Adding fresh constant for subsort "<<s<<std::endl;
#endif
    }
    if((_print)){
      cout << "Subsort " << s << " has " << _sig->sortedConstants[s].size() << " constants and ";
      cout << _sig->sortedFunctions[s].size() << " functions" <<std::endl;
    }
  }
  ...
  for(unsigned s=0;s<comps;s++){
    ...
    if(_sig->sortedFunctions[s].size()==0 && !posEqualitiesOnSort[s]){
      ...
      if(_print){
        cout << "Found bound of " << _sig->sortBounds[s] << " for subsort " << s << std::endl;
#if DEBUG_SORT_INFERENCE
    ...
    //if(s==3){
      //cout << "Forcing all bounds to max for " << s << std::endl;
      //bounds[s] = UINT_MAX;
    //}
  }
...
#if DEBUG_SORT_INFERENCE
  cout << "Setting function _signatures" << std::endl;
#endif
  ...
  for(unsigned f=0;f<env.signature->functions();f++){
    if(f < _del_f.size() && _del_f[f]) {
#if DEBUG_SORT_INFERENCE
    cout << "Skipping deleted function signature "  << env.signature->functionName(f) << std::endl;
#endif
      continue;
    }
#if DEBUG_SORT_INFERENCE
...
#if DEBUG_SORT_INFERENCE
      cout << " fresh constant, so skipping" << std::endl;
#endif
      continue;
    }
...
#if VDEBUG
      //cout << "FUNCTION " << env.signature->functionName(f) << std::endl;
...
#if DEBUG_SORT_INFERENCE
   cout << "("<< offset_f[f] << ")"<< std::endl;
#endif
  }
#if DEBUG_SORT_INFERENCE
  cout << "Setting up fresh constant info" << std::endl;
#endif
...
#if DEBUG_SORT_INFERENCE
  cout << "Setting predicate _signatures" << std::endl;
#endif
  ...
  if(_print){
    if(_collapsed>0){ cout << "Collapsed " << _collapsed << " distinct sorts into 1 as they are monotonic" << std::endl;}
    cout << _sig->distinctSorts << " distinct sorts" << std::endl;
    for(unsigned s=0;s<_sig->distinctSorts;s++){
      ...
      cout << s << " has " << children << " inferred subsorts as members [" << res << "]" << std::endl;
    }
    cout << "Vampire to distinct sort mapping:" << std::endl;
    cout << "["; 
    for(unsigned i=0;i<_sig->distinctSorts;i++){
      ...
      if(i==_sig->distinctSorts-1) cout << "]" << std::endl;
      else cout << ",";
    }
  }
...
#if DEBUG_SORT_INFERENCE 
      cout << "Parent " << parent << " for " << env.signature->typeConName(s) << std::endl;
#endif
...
#if DEBUG_SORT_INFERENCE 
        cout << "Child " << child << " for " << env.signature->typeConName(s) << std::endl;
#endif
...
unsigned SortInference::getDistinctSort(unsigned subsort, unsigned realVampireSort, bool createNew)
{
  ...
  if(_expandSubsorts){
    ...
    //cout << "CREATE " << subsort << "," << env.sorts->sortName(realVampireSort) << std::endl;
    ASS(createNew);
   ...
   //cout << "RET " << vampireSort << " to " << ourSort << std::endl;
