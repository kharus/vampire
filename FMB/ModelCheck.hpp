...
static void doCheck(Problem*& prb)
{
  ...
  Set<Term*> domainConstants;
  {
    ...
    while(uit.hasNext()){
      ...
      if(name == "finite_domain"){
        //cout << "Finite domain axiom found:" << std::endl << u->toString() << std::endl;
  ...
  cout << "Detected model of size " << modelSize << std::endl;
  cout << "Distinct domain assumed, domain elements are:" << std::endl;
...

  {
    ...
    cout << "Checking formulas..." << std::endl;
    {
      UnitList::Iterator uit(prb->units());
      while(uit.hasNext()){
        Unit* u = uit.next();
        ...
        cout << "Checking " << u->toString() << "..." << std::endl;
        bool res = model.evaluate(u);
        cout << "Evaluates to " << (res ? "True" : "False") << std::endl;
      }
    }
  }
}
