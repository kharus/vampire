...
namespace FMB {
  ...
  private:
    ...
    void process(Clause* c){
      //cout << "Process " << c->toString() << std::endl;
      ...
      for(unsigned i=0;i<c->length();i++){
        ...
        //cout << " process " << l->toString() << std::endl;
    ...
    Term* addGroundDefinition(Term* term, Clause* from){
      //cout << "Adding defs for " << term->toString() << std::endl;
      ASS(term->ground());
      ...
      while(it.hasNext() || retC==0){
        ...
        //cout << "Considering " << t->toString() << std::endl;
        ...
        if(!_introduced.find(t)){
          ...
          //cout << "creating def " << def->toString() << std::endl;
          _processed.push(def); 
        }
      }
      ASS(retC);
