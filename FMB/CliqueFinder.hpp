...
namespace FMB {
  ...
  public:
    ...
    {
      //cout << "findMaxCliqueSize with " << Ngraph->size() << std::endl;
      ...
      while(miter.hasNext()){
        ...
        //while(dit.hasNext()){ cout << dit.next() << std::endl; }

        for(;size>0;size--){
          atleast[size].push(c);
        }
      }
      //cout << "Searching" << std::endl;
      ...
      for(unsigned i=atleast.size()-1;i>1;i--){
        ...
        if(atleast[i].size() == i+1){
          ...
          if(checkClique(Ngraph,atleast[i])){
            //cout << "FIND(A) max clique of " << (i+1) << std::endl;
            //for(unsigned j=0;j<atleast[i].size();j++){ cout << atleast[i][j] << " ";}; cout << std::endl;
            return i+1;
          }
        }
          ...
          while(niter.hasNext() && left >= i+1){
            ...
            if(ns->size()==i){
              ...
              if(checkClique(Ngraph,clique)){
                //cout << "FIND(B) max clique of " << (i+1) << std::endl;
                return i+1;
              }
              left--;
            }
          }
          //cout << "In this case with " << left << " left" << std::endl;
        }
      }

      //for(unsigned i=1;i<atleast.size();i++){
      //  cout << i << ":"<< atleast[i].size() << std::endl; 
      //}
      return 1;
    }
  private:
    ...
    {
      //cout << "CHECK "; for(unsigned j=0;j<clique.size();j++){ cout << clique[j] << " ";}; cout << std::endl;

      for(unsigned i=0;i<clique.size()-1;i++){
        unsigned c1 = clique[i];
        auto ns = Ngraph->get(c1);
        //cout << c1 << " neighbours: "; 
        //DHSet<unsigned>::Iterator pit(*ns);while(pit.hasNext()){cout << pit.next() << " ";};cout<<std::endl;
        for(unsigned j=i+1;j<clique.size();j++){
          unsigned c2 = clique[j];
          //cout << "checking " << c2 << " is a neighbour of " << c1 << std::endl;
