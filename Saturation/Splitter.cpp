...
void SplittingBranchSelector::handleSatRefutation()
{
  ...
  if (!env.colorUsed) { // color oblivious, simple approach
    ...
    while (it1.hasNext()) {
      ...
      // cout << "SAT: " << scl->toString() << std::endl;
    ...
    //cout << colorCnts[0] << " " << colorCnts[1] <<  " " << colorCnts[2] << std::endl;
...
Clause* Splitter::buildAndInsertComponentClause(SplitLevel name, unsigned size, Literal* const * lits, Clause* orig)
{
  ...
  if (!_defs.find(posName, def_u)) {
    ...
    // cout << "Add def " << def_u->toString() << " for " << name << std::endl;
...
UnitList* Splitter::preprendCurrentlyAssumedComponentClauses(UnitList* clauses)
{
  ...
  while(ait.hasNext()) {
    ...
    //cout << "selected level: " level << " has clause: " << cl->toString() << std::endl;
    seen.insert(cl);
  ...
  while (uit.hasNext()) {
    ...
    if (seen.insert(cl)) {
      // cout << "a new guy: " << cl->toString() << std::endl;
      fifo.pushBack(cl);
    } else {
      // cout << "seen already: " << cl->toString() << std::endl;
    }
  }
